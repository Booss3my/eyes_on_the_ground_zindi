...
for i, (train_index, val_index) in enumerate(skf.split(data.index,data.extent)):
    ...
    train_dataset = EogDataset(train_image_paths, labels = data.extent[train_index],size=input_size, tfs=TRAIN_TFS)
    val_dataset = EogDataset(val_image_paths, labels =  data.extent[val_index],size=input_size,tfs=VAL_TFS)
    ...
    print_loss=one_epoch(base_model, train_dataloader, criterion, optimizer,type="train")
